...
class StaticMatmulModule4:
    @T.prim_func
    def main(
        A: T.Buffer[(1024, 2048), "float16"],
        B: T.Buffer[(2048, 1024), "float16"],
        C: T.Buffer[(1024, 1024), "float16"],
    ) -> None:
        # function attr dict
        T.func_attr({"global_symbol": "static_matmul_module_4", "tir.noalias": True})
        a0 = T.var("int32")
        a1 = T.var("int32")
        b0 = T.var("int32")
        b1 = T.var("int32")
        ...
        A_reindex_shared = T.alloc_buffer([1024, 2048], dtype="float16", scope="shared")
        B_reindex_shared = T.alloc_buffer([2048, 1024], dtype="float16", scope="shared")
        A_reindex_shared_wmma_matrix_a = T.alloc_buffer(
            [1024, 2048], dtype="float16", scope="wmma.matrix_a"
        )
        B_reindex_shared_wmma_matrix_b = T.alloc_buffer(
            [2048, 1024], dtype="float16", scope="wmma.matrix_b"
        )
