...
def apply_trace(sch: tir.Schedule) -> None:
    ...
    # )
    # v26, v27 = sch.sample_perfect_tile(loop=l21, n=2, max_innermost_factor=4, decision=[2, 2])
    # l29, l30, l31, l32, l33 = sch.split(
    #    loop=l21, factors=[v24, v25, v26, v27, v28], preserve_unit_iters=True
    # )
    # l31, l32 = sch.split(loop=l21, factors=[v26, v27], preserve_unit_iters=True)
    # v34, v35, v36, v37, v38 = sch.sample_perfect_tile(
    #    loop=l22, n=5, max_innermost_factor=4, decision=[16, 1, 2, 4, 1]  # TODO: 128
    # )
    # v36, v37 = sch.sample_perfect_tile(loop=l22, n=2, max_innermost_factor=4, decision=[2, 4])
    # l39, l40, l41, l42, l43 = sch.split(
    #    loop=l22, factors=[v34, v35, v36, v37, v38], preserve_unit_iters=True
    # )
    # l39, l40 = sch.split(loop=l22, factors=[v36, v37], preserve_unit_iters=True)
    ...
    # sch.reorder(l31, l39, l32, l40, l23)
    ...
    # l52 = sch.fuse(l31, l39, preserve_unit_iters=True)
    ...
    l78, l79 = sch.get_loops(block=b73)
    ...
    b91 = sch.cache_read(block=b20, read_buffer_index=1, storage_scope="wmma.matrix_a")
    # sch.compute_at(block=b91, loop=l48, preserve_unit_loops=True, index=-1)
    b113 = sch.cache_read(block=b20, read_buffer_index=2, storage_scope="wmma.matrix_b")
    # sch.compute_at(block=b113, loop=l48, preserve_unit_loops=True, index=-1)
    sch.tensorize(block_or_loop=b20, tensor_intrin="wmma_sync_16x16x16_f16f16f16_trans")

    """
    # l92, l93, l94, l95, l96, l97, l98 = sch.get_loops(block=b91)
    l97, l98 = sch.get_loops(block=b91)
    ...
    l108, l109, l110, l111 = sch.get_loops(block=b91)
    ...
    l119, l120 = sch.get_loops(block=b113)
    ...
    l130, l131, l132, l133 = sch.get_loops(block=b113)
    ...
    l138, l139 = sch.get_loops(block=b53)
    ...
    l148 = sch.get_loops(block=b73)[0]
    ...
    l157 = sch.get_loops(block=b82)[0]
    ...
    b169 = sch.get_child_blocks(b162)[0]
    ...
    l215, l216 = sch.get_loops(block=b169)
    ...
    b237 = sch.get_block(name="C", func_name="main")
...
def apply_trace_orig(sch: tir.Schedule) -> None:
    b0 = sch.get_block(name="C", func_name="main")
    b1 = sch.get_block(name="root", func_name="main")
    sch.annotate(block_or_loop=b0, ann_key="meta_schedule.tiling_structure", ann_val="SSSRRSRS")
    """
...
def apply_trace2(sch):
    # correct version (except for commented part)
    ...
    l1, l2, l3, l4 = sch.split(loop=l, factors=[None, 4, 32, 2], preserve_unit_iters=True)
    sch.vectorize(loop=l4)
    sch.bind(loop=l3, thread_axis="threadIdx.x")
    sch.bind(loop=l2, thread_axis="threadIdx.y")

    b1, b2 = sch.get_loops(B_shared)
    l = sch.fuse(b1, b2, preserve_unit_iters=True)
    l1, l2, l3, l4 = sch.split(loop=l, factors=[None, 4, 32, 4], preserve_unit_iters=True)
    sch.vectorize(loop=l4)
    sch.bind(loop=l3, thread_axis="threadIdx.x")
    sch.bind(loop=l2, thread_axis="threadIdx.y")

    c1, c2 = sch.get_loops(C_shared)
    l = sch.fuse(c1, c2, preserve_unit_iters=True)
    l1, l2, l3, l4 = sch.split(loop=l, factors=[None, 4, 32, 1], preserve_unit_iters=True)
    sch.vectorize(loop=l4)
    sch.bind(loop=l3, thread_axis="threadIdx.x")
    sch.bind(loop=l2, thread_axis="threadIdx.y")

    l1, l2 = sch.get_loops(A_wmma)
    l01, l02 = sch.split(loop=l2, factors=[None, 16], preserve_unit_iters=True)
    l03, l04 = sch.split(loop=l1, factors=[None, 16], preserve_unit_iters=True)
    l01, l02, l03, l04 = sch.get_loops(A_wmma)
    sch.reorder(l03, l02)
    sch.blockize(l02)

    l1, l2 = sch.get_loops(B_wmma)
    l01, l02 = sch.split(loop=l2, factors=[None, 16], preserve_unit_iters=True)
    l03, l04 = sch.split(loop=l1, factors=[None, 16], preserve_unit_iters=True)
    l01, l02, l03, l04 = sch.get_loops(B_wmma)
    sch.reorder(l03, l02)
    sch.blockize(l02)

    l1, l2 = sch.get_loops(C_wmma)
    l01, l02 = sch.split(loop=l2, factors=[None, 16], preserve_unit_iters=True)
    l03, l04 = sch.split(loop=l1, factors=[None, 16], preserve_unit_iters=True)
    l01, l02, l03, l04 = sch.get_loops(C_wmma)
    sch.reorder(l03, l02)
    sch.blockize(l02)

    # b = sch.get_block("A_pad_shared_wmma.matrix_a_o")
