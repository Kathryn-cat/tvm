...
class Microkernel_256_128_32:
    ...
    def main(
        ...
        # body
        # with T.block("root")
        C_reindex_shared = T.alloc_buffer([256, 128], dtype="float16", scope="shared")
        C_reindex_shared_wmma_accumulator = T.alloc_buffer(
            [256, 128], dtype="float16", scope="wmma.accumulator"
        )
        A_reindex_shared = T.alloc_buffer([256, 32], dtype="float16", scope="shared")
        B_reindex_shared = T.alloc_buffer([32, 128], dtype="float16", scope="shared")
        A_reindex_shared_wmma_matrix_a = T.alloc_buffer(
            [256, 32], dtype="float16", scope="wmma.matrix_a"
        )
                        ...
                        for ax2_0_1 in T.serial(1):
                            for ax0_0, ax1_0 in T.grid(1, 2):
                                with T.block("A_reindex_shared_wmma.matrix_a_o"):
                                    v0_o = T.axis.spatial(
                                        16, ax0_0_0_ax1_0_0_fused // 4 * 2 + ax0_0_1_ax1_0_1_fused
                                    )
...
def apply_trace(sch: tir.Schedule) -> None:
    ...
    v24, v25, v26, v27, v28 = sch.sample_perfect_tile(
        loop=l21, n=5, max_innermost_factor=4, decision=[8, 2, 1, 1, 1]
    )
    ...
    v34, v35, v36, v37, v38 = sch.sample_perfect_tile(
        loop=l22, n=5, max_innermost_factor=4, decision=[4, 1, 2, 1, 1]
    )
    ...
    l148, l149, l150, l151 = sch.split(
        loop=l147, factors=[None, 2, 32, 8], preserve_unit_iters=True
    )
...
------------------------------------------------------------------------------------------------------
  0 | main | 2097152 |      1 |       895.7821 |       2.3411 |                2.3411 |   1000 |    Y
------------------------------------------------------------------------------------------------------
Total trials: 1000
Total latency (us): 2.34114

best tuning record is 0, min_run_time is 2.341140651448278e-06
"""
