...
class Microkernel_256_64_32:
    ...
    def main(
        ...
        C_reindex_shared_wmma_accumulator = T.alloc_buffer(
            [256, 64], dtype="float16", scope="wmma.accumulator"
        )
                        ...
                        for ax2_0_1 in T.serial(1):
                            for ax0_0, ax1_0 in T.grid(1, 2):
                                with T.block("A_reindex_shared_wmma.matrix_a_o"):
                                    v0_o = T.axis.spatial(
                                        16, ax0_0_0_ax1_0_0_fused * 8 + ax0_0_1_ax1_0_1_fused // 2
                                    )
                            ...
                            for ax0_0, ax1_0 in T.grid(2, 1):
                                with T.block("B_reindex_shared_wmma.matrix_b_o"):
                                    ...
                                    v1_o = T.axis.spatial(
                                        4, ax0_0_1_ax1_0_1_fused % 2 * 2 + ax0_0_2_ax1_0_2_fused
                                    )
                                    T.reads(
                                        B_reindex_shared[
                                            v0_o * 16 : v0_o * 16 + 16, v1_o * 16 : v1_o * 16 + 16
...
def apply_trace(sch: tir.Schedule) -> None:
    ...
    v24, v25, v26, v27, v28 = sch.sample_perfect_tile(
        loop=l21, n=5, max_innermost_factor=4, decision=[2, 8, 1, 1, 1]
    )
    ...
    v34, v35, v36, v37, v38 = sch.sample_perfect_tile(
        loop=l22, n=5, max_innermost_factor=4, decision=[1, 2, 2, 1, 1]
    )
...
------------------------------------------------------------------------------------------------------
  0 | main | 1048576 |      1 |       462.1208 |       2.2691 |                2.2691 |   1000 |    Y
------------------------------------------------------------------------------------------------------
Total trials: 1000
Total latency (us): 2.26905

best tuning record is 0, min_run_time is 2.2690517515436817e-06
"""
