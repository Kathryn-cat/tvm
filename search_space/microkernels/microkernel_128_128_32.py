                ...
                for ax0_0_2_ax1_0_2_fused in T.thread_binding(2, thread="threadIdx.y"):
                                ...
                                for ax0_ax1_fused_2 in T.thread_binding(32, thread="threadIdx.x"):
                                    for ax0_ax1_fused_3 in T.vectorized(4):
                                        with T.block("A_reindex_shared"):
                                            v0 = T.axis.spatial(
                                                128,
                                                ax0_0_0_ax1_0_0_fused // 2 * 64
                                                + ax0_0_1_ax1_0_1_fused // 2 * 16
                                                + (
                                                    ax0_ax1_fused_0 * 256
                                                    + ax0_ax1_fused_1 * 128
                                                    + ax0_ax1_fused_2 * 4
                        ...
                        for ax2_0_1 in T.serial(2):
                            ...
                            for ax0_0_3, ax1_0_3, ax2_0_2, ax0_0_4, ax1_0_4 in T.grid(
                                ...
                                with T.block("C_o_update"):
                                    ...
                                    v2_o = T.axis.reduce(2, ax2_0_0 * 2 + ax2_0_1 + ax2_0_2)
                                    T.reads(
                                        C_reindex_shared_wmma_accumulator[
                                            v0_o * 16 : v0_o * 16 + 16, v1_o * 16 : v1_o * 16 + 16
                                        ],
                    ...
                    for ax0_0, ax1_0 in T.grid(1, 1):
                        with T.block("C_reindex_shared_wmma.accumulator_o"):
                            v0_o = T.axis.spatial(
                                8, ax0_0_0_ax1_0_0_fused // 2 * 4 + ax0_0_1_ax1_0_1_fused // 2
                            )
                            v1_o = T.axis.spatial(
                                8,
                                ax0_0_0_ax1_0_0_fused % 2 * 4
...
def sch_128_128_32_v1(sch: tir.Schedule) -> None:
    ...
    v26, v27, v28 = sch.sample_perfect_tile(
        loop=l21, n=3, max_innermost_factor=4, decision=[1, 1, 1]
    )
    ...
    v36, v37, v38 = sch.sample_perfect_tile(
        loop=l22, n=3, max_innermost_factor=4, decision=[2, 1, 1]
    )
    ...
    l148, l149, l150, l151 = sch.split(
        loop=l147, factors=[None, 2, 32, 4], preserve_unit_iters=True
    )
    ...
    sch.annotate(block_or_loop=l169, ann_key="pragma_auto_unroll_max_step", ann_val=64)
    ...
    sch.annotate(block_or_loop=l192, ann_key="pragma_auto_unroll_max_step", ann_val=64)
    ...
    sch.annotate(block_or_loop=l199, ann_key="pragma_auto_unroll_max_step", ann_val=64)
    ...
    sch.annotate(block_or_loop=l214, ann_key="pragma_auto_unroll_max_step", ann_val=64)
...
def sch_128_128_32_v2(sch: tir.Schedule, i0, j0) -> None:
    b0 = sch.get_block(name="C", func_name="main")
    b1 = sch.get_block(name="root", func_name="main")
    sch.annotate(block_or_loop=b0, ann_key="meta_schedule.tiling_structure", ann_val="SSSRRSRS")
    """
    ...
    b53 = sch.cache_write(block=b20, write_buffer_index=0, storage_scope="shared")
    # sch.reverse_compute_at(block=b53, loop=j0, preserve_unit_loops=True, index=-1)
    b54 = sch.cache_write(block=b20, write_buffer_index=0, storage_scope="wmma.accumulator")
    # sch.reverse_compute_at(block=b54, loop=l52, preserve_unit_loops=True, index=-1)
    """
    v55 = sch.sample_categorical(
...
------------------------------------------------------------------------------------------------------
  0 | main | 1048576 |      1 |       469.0186 |       2.2357 |                2.2357 |   1000 |    Y
------------------------------------------------------------------------------------------------------
Total trials: 1000
Total latency (us): 2.23568

best tuning record is 0, min_run_time is 2.235681201922339e-06
"""
