...
class Microkernel_64_256_32:
    ...
    def main(
        ...
        # body
        # with T.block("root")
        C_reindex_shared = T.alloc_buffer([64, 256], dtype="float16", scope="shared")
        C_reindex_shared_wmma_accumulator = T.alloc_buffer(
            [64, 256], dtype="float16", scope="wmma.accumulator"
        )
        A_reindex_shared = T.alloc_buffer([64, 32], dtype="float16", scope="shared")
        B_reindex_shared = T.alloc_buffer([32, 256], dtype="float16", scope="shared")
        ...
        B_reindex_shared_wmma_matrix_b = T.alloc_buffer(
            [32, 256], dtype="float16", scope="wmma.matrix_b"
        )
                ...
                for ax0_0_2_ax1_0_2_fused in T.thread_binding(2, thread="threadIdx.y"):
                        ...
                        for ax2_0_1 in T.serial(2):
                            for ax0_0, ax1_0 in T.grid(1, 1):
                                with T.block("A_reindex_shared_wmma.matrix_a_o"):
                                    v0_o, v1_o = T.axis.remap(
                                        "SS", [ax0_0_0_ax1_0_0_fused, ax2_0_1]
                                    )
                                    T.reads(
                                        A_reindex_shared[
                                            v0_o * 16 : v0_o * 16 + 16, v1_o * 16 : v1_o * 16 + 16
                            ...
                            for ax0_0, ax1_0 in T.grid(1, 1):
                                with T.block("B_reindex_shared_wmma.matrix_b_o"):
                                    ...
                                    v1_o = T.axis.spatial(
                                        16, ax0_0_1_ax1_0_1_fused * 2 + ax0_0_2_ax1_0_2_fused
                                    )
                                    T.reads(
                                        B_reindex_shared[
                                            v0_o * 16 : v0_o * 16 + 16, v1_o * 16 : v1_o * 16 + 16
                    ...
                    for ax0_0, ax1_0 in T.grid(1, 1):
                        with T.block("C_reindex_shared_wmma.accumulator_o"):
                            v0_o = T.axis.spatial(4, ax0_0_0_ax1_0_0_fused)
                            v1_o = T.axis.spatial(
                                16, ax0_0_1_ax1_0_1_fused * 2 + ax0_0_2_ax1_0_2_fused
                            )
                            T.reads(
                    ...
                    for ax1_1 in T.thread_binding(2, thread="threadIdx.y"):
                        for ax1_2 in T.thread_binding(32, thread="threadIdx.x"):
                            with T.block("C_reindex_shared"):
                                ...
                                v1 = T.axis.spatial(
                                    256,
                                    ax0_0_1_ax1_0_1_fused * 32 + (ax1_0 * 64 + ax1_1 * 32 + ax1_2),
                                )
...
def apply_trace(sch: tir.Schedule) -> None:
    ...
    v34, v35, v36, v37, v38 = sch.sample_perfect_tile(
        loop=l22, n=5, max_innermost_factor=4, decision=[1, 8, 2, 1, 1]
    )
    ...
    l157, l158, l159, l160 = sch.split(
        loop=l156, factors=[None, 2, 32, 8], preserve_unit_iters=True
    )
...
------------------------------------------------------------------------------------------------------
  0 | main | 1048576 |      1 |       468.5196 |       2.2381 |                2.2381 |   1000 |    Y
------------------------------------------------------------------------------------------------------
Total trials: 1000
Total latency (us): 2.23806

best tuning record is 0, min_run_time is 2.238062021326735e-06
"""
