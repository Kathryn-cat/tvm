...
class Microkernel_128_256_32:
    ...
    def main(
        ...
        # body
        # with T.block("root")
        C_reindex_shared = T.alloc_buffer([128, 256], dtype="float16", scope="shared")
        C_reindex_shared_wmma_accumulator = T.alloc_buffer(
            [128, 256], dtype="float16", scope="wmma.accumulator"
        )
                ...
                for ax0_0_2_ax1_0_2_fused in T.thread_binding(1, thread="threadIdx.y"):
                        ...
                        for ax2_0_1 in T.serial(1):
                            for ax0_0, ax1_0 in T.grid(1, 2):
                                with T.block("A_reindex_shared_wmma.matrix_a_o"):
                                    v0_o = T.axis.spatial(
                                        8,
                                        ax0_0_0_ax1_0_0_fused // 4 * 4 + ax0_0_1_ax1_0_1_fused // 2,
                                    )
                    ...
                    for ax0_0, ax1_0 in T.grid(1, 2):
                        with T.block("C_reindex_shared_wmma.accumulator_o"):
                            v0_o = T.axis.spatial(
                                8, ax0_0_0_ax1_0_0_fused // 4 * 4 + ax0_0_1_ax1_0_1_fused // 2
                            )
                            v1_o = T.axis.spatial(
                                16,
                                ax0_0_0_ax1_0_0_fused % 4 * 4
...
def apply_trace(sch: tir.Schedule) -> None:
    ...
    v34, v35, v36, v37, v38 = sch.sample_perfect_tile(
        loop=l22, n=5, max_innermost_factor=4, decision=[4, 2, 1, 2, 1]
    )
...
------------------------------------------------------------------------------------------------------
  0 | main | 2097152 |      1 |       873.8061 |       2.4000 |                2.4000 |   1000 |    Y
------------------------------------------------------------------------------------------------------
Total trials: 1000
Total latency (us): 2.40002

best tuning record is 0, min_run_time is 2.400019916360898e-06
"""
